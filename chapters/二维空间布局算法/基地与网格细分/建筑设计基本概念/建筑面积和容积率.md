# 建筑面积和容积率

## 开始做实体实验

![](/images/二维空间布局算法/基地与网格细分/建筑设计基本概念/建筑面积和容积率/1a1.jpg)
![](/images/二维空间布局算法/基地与网格细分/建筑设计基本概念/建筑面积和容积率/2a1.jpg)
![](/images/二维空间布局算法/基地与网格细分/建筑设计基本概念/建筑面积和容积率/3a1.jpg)
![](/images/二维空间布局算法/基地与网格细分/建筑设计基本概念/建筑面积和容积率/4a1.jpg)

```python
import matplotlib.pyplot as plt

# ===============================
# 设置绘图比例相等，保证矩形比例正确
# ===============================
plt.axis('equal')

# ===============================
# 矩形类
# ===============================
class Rectangle:
    """矩形类：通过左下角坐标和尺寸定义矩形"""
    def __init__(self, x, y, length, width):
        """
        初始化矩形
        参数：
        x, y - 矩形左下角坐标
        length - 矩形在X方向长度
        width  - 矩形在Y方向宽度
        """
        self.x = x
        self.y = y
        self.length = length
        self.width = width        

    # ===========================
    # 获取属性方法
    # ===========================
    def getX(self): return self.x
    def getY(self): return self.y
    def getLength(self): return self.length
    def getWidth(self): return self.width

    # ===========================
    # 计算面积
    # ===========================
    def area(self):
        """返回矩形面积"""
        return self.length * self.width

    # ===========================
    # 获取中心点坐标
    # ===========================
    def centralCoordinates(self):
        """返回矩形中心点坐标 [x_center, y_center]"""
        x_center = self.x + self.length / 2
        y_center = self.y + self.width / 2
        return [x_center, y_center]

    # ===========================
    # 绘制矩形
    # ===========================
    def render(self):
        """在当前Matplotlib图中绘制矩形"""
        p1 = [self.x, self.y]  # 左下角
        p2 = [self.x + self.length, self.y]  # 右下角
        p3 = [self.x + self.length, self.y + self.width]  # 右上角
        p4 = [self.x, self.y + self.width]  # 左上角

        # 绘制四条边
        plt.plot([p1[0], p2[0]], [p1[1], p2[1]], color="green")
        plt.plot([p2[0], p3[0]], [p2[1], p3[1]], color="green")
        plt.plot([p3[0], p4[0]], [p3[1], p4[1]], color="green")
        plt.plot([p4[0], p1[0]], [p4[1], p1[1]], color="green")	

# ===============================
# 1. 创建建设用地矩形并绘制
# ===============================
x_of_site = -13 
y_of_site = -9
length_of_site = 37
width_of_site = 28

rect_of_site = Rectangle(x_of_site, y_of_site, length_of_site, width_of_site)
rect_of_site.render()
print("建设用地面积是", rect_of_site.area(), "平方米。")

# ===============================
# 2. 创建建筑红线矩形（距离用地边界property_line_offset）
# ===============================
property_line_offset = 2  # 与建设用地边界的距离

x_of_property_line_rec = x_of_site + property_line_offset
y_of_property_line_rec = y_of_site + property_line_offset
length_of_property_line_rec = length_of_site - property_line_offset * 2
width_of_property_line_rec = width_of_site - property_line_offset * 2

rect_of_property_line_rec = Rectangle(
    x_of_property_line_rec,
    y_of_property_line_rec,
    length_of_property_line_rec,
    width_of_property_line_rec
)
rect_of_property_line_rec.render()
print("建筑红线面积是", rect_of_property_line_rec.area(), "平方米。")

# ===============================
# 3. 创建建筑分区（每层楼或每个功能区）
# ===============================
zone1 = Rectangle(-7.8, 0, 3.9, 8.1)
zone2 = Rectangle(-3.9, 0, 3.9, 8.1)
zone3 = Rectangle(0, 0, 3.9, 8.1)
zone4 = Rectangle(3.9, 0, 3.9, 8.1)
zone5 = Rectangle(7.8, 0, 3.9, 8.1)
zone6 = Rectangle(11.7, 0, 3.9, 8.1)

zones = [zone1, zone2, zone3, zone4, zone5, zone6]

# ===============================
# 4. 绘制分区并计算总建筑面积
# ===============================
total_floor_area = 0  # 初始化总建筑面积

for zone in zones:
    zone.render()  # 绘制分区
    total_floor_area += zone.area()  # 累加面积

print("建筑面积是", total_floor_area, "平方米。")

# ===============================
# 5. 计算容积率
# ===============================
floor_area_ratio = total_floor_area / rect_of_site.area()
print("容积率是", round(floor_area_ratio, 2))

# ===============================
# 显示绘图
# ===============================
plt.show()
```

## 参考文献及资料

1. [Floor area](https://en.wikipedia.org/wiki/Floor_area) | [建筑面积](https://zh.wikipedia.org/wiki/%E5%BB%BA%E7%AD%91%E9%9D%A2%E7%A7%AF)
2. [Floor area ratio](https://en.wikipedia.org/wiki/Floor_area_ratio) | [容积率](https://zh.wikipedia.org/wiki/%E5%AE%B9%E7%A7%AF%E7%8E%87) 
3. [**For loops**](https://wiki.python.org/moin/ForLoop) 