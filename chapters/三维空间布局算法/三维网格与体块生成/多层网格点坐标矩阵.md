# 多层网格点坐标矩阵

### 打印首层和多层多个点坐标

```python
import matplotlib.pyplot as plt

ax = plt.figure().add_subplot(111, projection='3d')

# ---------------- 基地尺寸与顶点定义 ----------------
length = 240
width = 200

site = [
    [0, 0, 0],          # 左下角
    [length, 0, 0],     # 右下角
    [length, width, 0], # 右上角
    [0, width, 0]       # 左上角
]  # print(site)  # 保留硬编码注释，用于调试查看

# ---------------- 绘制方法（基地边界） ----------------
ax.plot([site[0][0], site[1][0]], [site[0][1], site[1][1]], [site[0][2], site[1][2]], color="green")
ax.plot([site[1][0], site[2][0]], [site[1][1], site[2][1]], [site[1][2], site[2][2]], color="green")
ax.plot([site[2][0], site[3][0]], [site[2][1], site[3][1]], [site[2][2], site[3][2]], color="green")
ax.plot([site[3][0], site[0][0]], [site[3][1], site[0][1]], [site[3][2], site[0][2]], color="green")

# ---------------- 网格采样参数 ----------------
distance_in_length = 16
distance_in_width = 16

segments_in_length = int(length / distance_in_length)   # print(segments_in_length)
segments_in_width = int(width / distance_in_width)      # print(segments_in_width)

# ---------------- 生成X方向采样点 ----------------
xArrayInLength = []
for i in range(1, segments_in_length):
    xArrayInLength.append(distance_in_length * i)

# ---------------- 生成Y方向采样点 ----------------
yArrayInWidth = []
for i in range(1, segments_in_width):
    yArrayInWidth.append(distance_in_width * i)

# ---------------- 初始化交点矩阵 ----------------
intersections = []
for _ in range(segments_in_width - 1):
    intersections.append([])

print(intersections)  # 空矩阵结构

# ---------------- 楼层高度定义 ----------------
height_of_single_floor = 4
number_of_1st_floor = 0
number_of_2nd_floor = 1
number_of_3rd_floor = 2

height_of_1st_floor = height_of_single_floor * number_of_1st_floor
height_of_2nd_floor = height_of_single_floor * number_of_2nd_floor
height_of_3rd_floor = height_of_single_floor * number_of_3rd_floor

# ---------------- 生成1层楼交点 ----------------
for j in range(segments_in_width - 1):
    for i in range(1, segments_in_length):
        intersections[j].append([
            xArrayInLength[i - 1],
            yArrayInWidth[j],
            height_of_1st_floor
        ])

print(intersections)
print(len(intersections))

# ---------------- 绘制方法（1层楼：红色点） ----------------
for j in range(len(intersections)):
    for i in range(len(intersections[j])):
        ax.scatter(
            intersections[j][i][0],
            intersections[j][i][1],
            intersections[j][i][2],
            c="red"
        )

# ---------------- 生成2层楼交点 ----------------
intersections2 = []
for row in intersections:
    intersections2.append(row.copy())

for j in range(segments_in_width - 1):
    for i in range(1, segments_in_length):
        intersections2[j].append([
            xArrayInLength[i - 1],
            yArrayInWidth[j],
            height_of_2nd_floor
        ])

# ---------------- 绘制方法（2层楼：绿色点） ----------------
for j in range(len(intersections2)):
    for i in range(len(intersections2[j])):
        ax.scatter(
            intersections2[j][i][0],
            intersections2[j][i][1],
            intersections2[j][i][2],
            c="green"
        )

# ---------------- 生成3层楼交点 ----------------
intersections3 = []
for row in intersections:
    intersections3.append(row.copy())

for j in range(segments_in_width - 1):
    for i in range(1, segments_in_length):
        intersections3[j].append([
            xArrayInLength[i - 1],
            yArrayInWidth[j],
            height_of_3rd_floor
        ])

# ---------------- 绘制方法（3层楼：蓝色点） ----------------
for j in range(len(intersections3)):
    for i in range(len(intersections3[j])):
        ax.scatter(
            intersections3[j][i][0],
            intersections3[j][i][1],
            intersections3[j][i][2],
            c="blue"
        )

# ---------------- 显示3D图形 ----------------
plt.show()
```